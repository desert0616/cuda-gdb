...
handle_cuda_autostep_event (void)
{
  ...
  cuda_initialize_autostep (stop_pc);
  ...
  if (b->location != NULL)
    {
      ...
      found = cuda_find_pc_from_address_string (objfile,
						(char *) event_location_to_string (b->location),
						&addr);
    }
			     ...
			     struct symtabs_and_lines sals_end)
{
  ...
  if (b->loc && b->loc->cuda_breakpoint)
    return;
  ...
  if (!*bp)
    {
      ...
      *bp = create_internal_breakpoint (cuda_gdbarch, addr, bp_cuda_auto,
      ...
      /* location has to be used or breakpoint_re_set will delete me.  */
      tmp  = xstrprintf ("*%s", paddress (cuda_gdbarch, (*bp)->loc->address));
      cleanup = make_cleanup (xfree, tmp);
      (*bp)->location = new_linespec_location (&tmp);
      do_cleanups (cleanup);
