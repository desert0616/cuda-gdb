...
/* Whether this is the last thread to be resumed.  */
int last_thread = 0;
...
proceed (CORE_ADDR addr, enum gdb_signal siggnal)
{
  ...
  /* CUDA - sstep */
  cuda_sstep_initialize (tp->control.stepping_command);
  ...
  if (step_over_info_valid_p ())
  ...
  else if (!tp->resumed && !thread_is_in_step_over_chain (tp))
    {
      ...
      reset_ecs (ecs, tp);
      switch_to_thread_keep_cuda_focus (tp->ptid);
      keep_going_pass_signal (ecs);
      if (!ecs->wait_some_more)
	error (_("Command aborted."));
    }
  ...
  if (!non_stop)
    {
	  ...
	  switch_to_thread_keep_cuda_focus (thr->ptid);
	  thread_fsm_clean_up (thr->thread_fsm, thr);
	}

      if (ecs->event_thread != NULL)
	switch_to_thread_keep_cuda_focus (ecs->event_thread->ptid);
    }
}
...
fetch_inferior_event (void *client_data)
{
  ...
  /* CUDA - focus - Only update previous_cuda_coords when we've had a valid
     event to work with.  If we got no event from the inferior, don't do
     anything.

     This is needed because newer GDB's use non-blocking waits for inferior
     events, and it may have happened that we got nothing out of the wait,
     like TARGET_WAITKIND_IGNORE.  */
  if (valid_event)
    cuda_coords_get_current (&previous_cuda_coords);
  ...
  if (!random_signal && cuda_get_autostep_pending ())
    {
      ...
      cuda_update_autostep_state (stop_pc);
