...
void
context_unload_elf_images (context_t this)
{
  gdb_assert (this);

  cuda_trace ("unloading ELF images of context 0x%016"PRIx64, this->context_id);

  modules_unload_elf_images (this->modules);
}

void
context_load_elf_images (context_t this)
{
  gdb_assert (this);

  cuda_trace ("loading ELF images of context 0x%016"PRIx64, this->context_id);

  modules_load_elf_images (this->modules);
}
  ...
  for (elt = this->list; elt; elt = elt->next)
    {
      ...
      modules_resolve_breakpoints (modules, context_id);
    }
...
void
contexts_cleanup_breakpoints (contexts_t this)
{
  list_elt_t elt;
  context_t  context;
  uint64_t   context_id;
...
{
  if (current_context.current &&
      context != current_context.current)
    context_unload_elf_images (current_context.current);

  current_context.current = context;

  if (context)
    context_load_elf_images (context);
}

void
save_current_context (void)
{
  gdb_assert (!current_context.saved_p);

  current_context.saved   = current_context.current;
  current_context.saved_p = true;
}

void
restore_current_context (void)
{
  gdb_assert (current_context.saved_p);

  set_current_context (current_context.saved);
  current_context.saved_p = false;
}
