...
static CORE_ADDR
dwarf_expr_read_addr_from_reg (void *baton, reg_t dwarf_regnum)
{
	...
	case DWARF_VALUE_REGISTER:
	  {
	    ...
	    if (gdbarch_bfd_arch_info (arch)->arch == bfd_arch_m68k)
	      ...
	      {
		dwarf_regnum = longest_to_int (dwarf_regnum);
...
dwarf2_evaluate_loc_desc (struct type *type, struct frame_info *frame,
			  const gdb_byte *data, size_t size,
			  struct dwarf2_per_cu_data *per_cu, CORE_ADDR push_obj)
{
...
static CORE_ADDR
symbol_needs_read_addr_from_reg (void *baton, reg_t regnum)
{
