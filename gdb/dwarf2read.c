...
static void dwarf2_start_subfile (const char *, const char *, struct objfile *);
...
setup_type_unit_groups (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (first_time)
      ...
      for (i = 0; i < lh->num_file_names; ++i)
	{
	  ...
	  dwarf2_start_subfile (fe->name, dir, objfile);
...
read_structure_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  /* Load __half struct as float */
  if (name && !strcmp(name, "__half"))
  {
    type = init_type (TYPE_CODE_FLT, 2, 0, NULL, cu->objfile);
    TYPE_NAME (type) = name;
...
process_structure_scope (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (type_name && !strcmp(type_name, "__half"))
      return;
...
make_loclist_baton (struct dwarf2_cu *cu, struct dwarf_block *blk)
{
    ...
    baton = (struct dwarf2_loclist_baton *) obstack_alloc (&cu->objfile->objfile_obstack,
  ...
  if (attr)
    {
       if (attr->form == DW_FORM_block1) 
 	{
	  ...
	  baton->size = size + 1;
 	  ...
 	  baton->data = (const gdb_byte *) obstack_alloc (&cu->objfile->objfile_obstack,
							  baton->size);
		  ...
		  const gdb_byte *info_ptr)
{
  ...
  if (has_program_calling_convention)
    set_objfile_main_name (objfile,
			   part_die->linkage_name ? part_die->linkage_name : part_die->name,
			   cu->language);
...
static void
dwarf2_start_subfile (const char *filename, const char *dirname, struct objfile *objfile)
{
  char *copy = NULL;
...
cuda_decode_line_header (unsigned int offset, struct objfile *objfile)
{
  ...
  line_ptr = (gdb_byte *) dwarf2_per_objfile->line.buffer + offset;
  ...
  lh->standard_opcode_lengths
    = (unsigned char *) xmalloc (lh->opcode_base * sizeof (lh->standard_opcode_lengths[0]));
  ...
  /* Read directory table.  */
  while ((cur_dir = (char *) read_direct_string (abfd, line_ptr, &bytes_read)) != NULL)
    {
      line_ptr += bytes_read;
      add_include_dir (lh, cur_dir);
    }
  ...
  /* Read file name table.  */
  while ((cur_file = (char *) read_direct_string (abfd, line_ptr, &bytes_read)) != NULL)
    {
      unsigned int dir_index, mod_time, length;
...
cuda_decode_lines (struct line_header *lh, struct objfile *objfile)
{
  ...
  line_ptr = (gdb_byte *) lh->statement_program_start;
  line_end = (gdb_byte *) lh->statement_program_end;
  ...
  while (line_ptr < line_end)
  {
    ...
    if (lh->num_file_names >= file)
    {
      ...
      if (fe->dir_index)
        dir = (char *) lh->include_dirs[fe->dir_index - 1];
            ...
            case DW_LNE_define_file:
              {
                ...
                cur_file = (char *) read_direct_string (abfd, line_ptr, &bytes_read);
                line_ptr += bytes_read;
                dir_index =
                  read_unsigned_leb128 (abfd, line_ptr, &bytes_read);
                line_ptr += bytes_read;
  ...
  for (i = 0; i < lh->num_file_names; i++)
  {
    ...
    if (current_subfile->symtab == NULL)
      {
	struct compunit_symtab *cust
	  = allocate_compunit_symtab (objfile, current_subfile->name);
...
static void
cuda_start_subfile(struct line_header *lh, int idx, struct objfile *objfile)
{
  const char *dir = NULL;
  struct file_entry *fe = NULL;
  fe = &lh->file_names[idx];
  if (fe->dir_index)
    dir = lh->include_dirs[fe->dir_index - 1];
  dwarf2_start_subfile(fe->name, dir, objfile);
}
...
static bool
does_msym_belong_to_line_table(struct minimal_symbol *msym,
                            struct line_header *lh,
                            struct objfile *objfile)
{
  CORE_ADDR sym_start, sym_end, sf_start, sf_end;
  int i, j, nitems;

  sym_start = MSYMBOL_VALUE_ADDRESS(objfile, msym);
  sym_end = sym_start + msym->size;
  for (i = 0; i < lh->num_file_names; ++i)
  {
    cuda_start_subfile(lh, i, objfile);
    if (!current_subfile->line_vector)
      continue;
    nitems = current_subfile->line_vector->nitems;
    if (nitems == 0)
      continue;
    /* Assuming the addresses in line_vector are sorted in ascending order,
     * it is sufficient to check that there is an overlap between msymbol range
     * and first and last line addresses in subfile line_vector */
    sf_start = current_subfile->line_vector->item[0].pc;
    sf_end = current_subfile->line_vector->item[nitems-1].pc;
    if (sf_end >= sym_start && sf_start < sym_end)
      return true;
  }
  return false;
}
...
allocate_blockvector (struct obstack *obstack, int nblocks)
{
  ...
  /* Static block*/
  blk = allocate_block (obstack);
  BLOCK_SUPERBLOCK(blk) = BLOCKVECTOR_BLOCK(bv, 0);
  BLOCKVECTOR_BLOCK (bv, 1) = blk;
  BLOCK_DICT (blk) = dict_create_hashed (obstack, NULL);
...
cuda_populate_blockvectors (struct line_header *lh,
                            struct objfile *objfile)
{
  int i = 0, j = 0, nblocks = 0, actual_nblocks = 2;
  struct block *block, *global_block = NULL;
  struct blockvector *blockvector = NULL;
  struct minimal_symbol *msym = NULL;
  struct symbol *sym = NULL;
  struct compunit_symtab *first_symtab = NULL;
  struct obstack *obstack = &objfile->objfile_obstack;
  struct pending *pending_symbols = NULL;

  /* First two blocks in the vector are global and static blocks */
  nblocks = objfile->per_bfd->minimal_symbol_count + 2;
  ...
  for (i = 0; i < lh->num_file_names; i++)
    {
      ...
      if (current_subfile->symtab)
	{
	  if (!SYMTAB_BLOCKVECTOR (current_subfile->symtab))
	    {
	      if (blockvector == NULL)
		blockvector = allocate_blockvector (obstack, nblocks);
	      SYMTAB_BLOCKVECTOR (current_subfile->symtab) = blockvector;
	    }

	  if (first_symtab == NULL)
	    first_symtab = SYMTAB_COMPUNIT (current_subfile->symtab);
	}
    }
  ...
  if (blockvector == NULL)
    blockvector = (struct blockvector *) SYMTAB_BLOCKVECTOR (current_subfile->symtab);
  global_block = BLOCKVECTOR_BLOCK (blockvector, 0);

  set_block_compunit_symtab (global_block, first_symtab); /* dummy */

  for (j = 2; j < nblocks; ++j)
    {
      msym = &objfile->per_bfd->msymbols[j-2];
      if (!does_msym_belong_to_line_table(msym, lh, objfile))
        continue;
	 ...
	 objfile.  */
      ...
      SYMBOL_LANGUAGE (sym) = MSYMBOL_LANGUAGE (msym);
      SYMBOL_BLOCK_VALUE (sym) = block;
      symbol_set_symtab (sym, COMPUNIT_FILETABS (first_symtab)); /* dummy */
      SYMBOL_SET_NAMES (sym, MSYMBOL_LINKAGE_NAME (msym),
                        strlen (MSYMBOL_LINKAGE_NAME (msym)), 1, objfile);
      add_symbol_to_list (sym, &pending_symbols);

      BLOCK_START (block) = MSYMBOL_VALUE_ADDRESS (objfile, msym);
      BLOCK_END (block) = MSYMBOL_VALUE_ADDRESS (objfile, msym) + MSYMBOL_SIZE (msym);
      BLOCK_FUNCTION (block) = sym;
      BLOCK_SUPERBLOCK (block) = global_block;
      BLOCK_DICT (block) = dict_create_hashed (obstack, NULL);

      BLOCKVECTOR_BLOCK (blockvector, actual_nblocks) = block;
      actual_nblocks++;
    }
  BLOCKVECTOR_NBLOCKS (blockvector) = actual_nblocks;
...
cuda_decode_line_table (struct objfile *objfile)
{
  ...
  /* We are going to start a new symtab, make sure we initialize the buildsym
     data structures.  We must make sure this happens otherwise we will end
     up with global variables in an inconsistent state, leading to internal
     errors.  */
  buildsym_init ();
  cleanups = make_cleanup (really_free_pendings, NULL);

  start_symtab (objfile, objfile->original_name, NULL, 0);
  ...
  do
    {
      ...
      if (lh == NULL)
	{
	  /* Make sure we reset the state of the global variables related
	     to assembling of symbol tables.  */
	  do_cleanups (cleanups);
	  return;
	}
  ...
  /* Reset everything back to the way it was, resetting the global variables
     involved in the process of building the symbol tables.  */
  do_cleanups (cleanups);
}
