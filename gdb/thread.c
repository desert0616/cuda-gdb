...
static void
switch_to_thread_1 (ptid_t ptid, int keep_cuda_focus)
{
  struct inferior *inf;
...
/* Switch from one thread to another and invalidate the CUDA device focus.  */

void
switch_to_thread (ptid_t ptid)
{
  switch_to_thread_1 (ptid, 0);
}

/* Switch from one thread to another but keep the CUDA device focus intact.  */

void
switch_to_thread_keep_cuda_focus (ptid_t ptid)
{
  switch_to_thread_1 (ptid, 1);
}

static void
restore_current_thread (ptid_t ptid)
{
  switch_to_thread_keep_cuda_focus (ptid);
}
