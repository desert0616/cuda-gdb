...
bppy_init (PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if (! gdbpy_ArgParseTupleAndKeywords (args, kwargs, "s|iiOO", keywords,
  ...
  CATCH (except, RETURN_MASK_ALL)
    {
      ...
      gdbpy_ErrFormat (except.reason == RETURN_QUIT
		       ? *pgdbpyExc_KeyboardInterrupt : *pgdbpyExc_RuntimeError,
		       "%s", except.message);
      return -1;
    }
  END_CATCH
...
local_setattro (PyObject *self, PyObject *name, PyObject *v)
{
  ...
  if (strcmp (attr, stop_func) == 0)
      ...
      if (extlang != NULL)
	{
	  ...
	  PyErr_SetString (gdbpyExc_RuntimeError, error_text);
	  xfree (error_text);
	  return -1;
	}
    }
