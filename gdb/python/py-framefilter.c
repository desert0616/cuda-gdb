...
{
  PyObject *result = gdbpy_ObjectCallMethod (obj, "symbol", NULL);
...
extract_value (PyObject *obj, struct value **value)
{
  if (PyObject_HasAttrString (obj, "value"))
    {
      PyObject *vresult = gdbpy_ObjectCallMethod (obj, "value", NULL);
...
get_py_iter_from_func (PyObject *filter, char *func)
{
  if (PyObject_HasAttrString (filter, func))
    {
      PyObject *result = gdbpy_ObjectCallMethod (filter, func, NULL);
		...
		struct ui_out *out, int indent, htab_t levels_printed)
{
  ...
  py_inf_frame = gdbpy_ObjectCallMethod (filter, "inferior_frame", NULL);
  ...
  if (print_frame_info)
	...
	{
	  PyObject *paddr = gdbpy_ObjectCallMethod (filter, "address", NULL);
	  ...
	  if (paddr != gdbpy_None)
	    {
	      address = gdbpy_Long_AsLong (paddr);
	      has_addr = 1;
	    }
	  Py_DECREF (paddr);
	}
  ...
  if (print_frame_info)
	...
	{
	  PyObject *py_func = gdbpy_ObjectCallMethod (filter, "function", NULL);
	  ...
	  else if (gdbpy_LongCheck (py_func))
	    {
  ...
  if (print_frame_info)
	...
	{
	  PyObject *py_fn = gdbpy_ObjectCallMethod (filter, "filename", NULL);
	...
	{
	  PyObject *py_line = gdbpy_ObjectCallMethod (filter, "line", NULL);
	  ...
	  if (py_line != gdbpy_None)
	    {
	      line = gdbpy_Long_AsLong (py_line);
	      TRY
		{
		  ui_out_text (out, ":");
				...
				int frame_low, int frame_high)
{
  ...
  iterable = gdbpy_ObjectCallFunctionObjArgs (sort_func, frame_obj,
					   py_frame_low,
					   py_frame_high,
					   NULL);
  if (iterable == NULL)
