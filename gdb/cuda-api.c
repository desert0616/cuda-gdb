...
cuda_api_initialize (void)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_initialize ();
  else
...
cuda_api_handle_initialization_error (CUDBGResult res)
{
  ...
  switch (res)
    ...
    case CUDBG_ERROR_INCOMPATIBLE_API:
      if (cudbgGetAPIVersion (&major, &minor, &revision) == CUDBG_SUCCESS)
        {
          char msg[1024];
...
cuda_api_resume_device (uint32_t dev)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_resume_device (dev);
  else
...
cuda_api_suspend_device (uint32_t dev)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_suspend_device (dev);
  else
...
cuda_api_single_step_warp (uint32_t dev, uint32_t sm, uint32_t wp, uint64_t *warp_mask)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_single_step_warp (dev, sm, wp, warp_mask);
  else
...
cuda_api_set_breakpoint (uint32_t dev, uint64_t addr)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_set_breakpoint (dev, addr);
  else
...
cuda_api_unset_breakpoint (uint32_t dev, uint64_t addr)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_unset_breakpoint (dev, addr);
  else
...
cuda_api_read_grid_id (uint32_t dev, uint32_t sm, uint32_t wp, uint64_t *grid_id)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_grid_id (dev, sm, wp, grid_id);
  else
...
cuda_api_read_block_idx (uint32_t dev, uint32_t sm, uint32_t wp, CuDim3 *blockIdx)
{
  ...
  if (cuda_remote)
    res =  cuda_remote_api_read_block_idx (dev, sm, wp, blockIdx);
  else
...
cuda_api_read_thread_idx (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, CuDim3 *threadIdx)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_thread_idx (dev, sm, wp, ln, threadIdx);
  else
...
cuda_api_read_broken_warps (uint32_t dev, uint32_t sm, uint64_t *brokenWarpsMask)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_broken_warps (dev, sm, brokenWarpsMask);
  else
...
cuda_api_read_valid_warps (uint32_t dev, uint32_t sm, uint64_t *valid_warps)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_valid_warps (dev, sm, valid_warps);
  else
...
cuda_api_read_valid_lanes (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t *valid_lanes)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_valid_lanes (dev, sm, wp, valid_lanes);
  else
...
cuda_api_read_active_lanes (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t *active_lanes)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_active_lanes (dev, sm, wp, active_lanes);
  else
...
cuda_api_read_code_memory (uint32_t dev, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_code_memory (dev, addr, buf, sz);
  else
...
cuda_api_read_const_memory (uint32_t dev, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_const_memory (dev, addr, buf, sz);
  else
...
cuda_api_read_global_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_global_memory (dev, sm, wp, ln, addr, buf, sz);
  else
  ...
  if (res == CUDBG_ERROR_ADDRESS_NOT_IN_DEVICE_MEM)
    {
      if (cuda_remote)
        res = cuda_remote_api_get_host_addr_from_device_addr (dev, addr, &hostaddr);
      else
...
cuda_api_read_pinned_memory (uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_pinned_memory (addr, buf, sz);
  else
...
cuda_api_read_param_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_param_memory (dev, sm, wp, addr, buf, sz); 
  else
...
cuda_api_read_shared_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_shared_memory (dev, sm, wp, addr, buf, sz); 
  else
...
cuda_api_read_texture_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t id, uint32_t dim, uint32_t *coords, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_texture_memory (dev, sm, wp, id, dim, coords, buf, sz); 
  else
...
cuda_api_read_texture_memory_bindless (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t tex_symtab_index, uint32_t dim, uint32_t *coords, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_texture_memory_bindless (dev, sm, wp, tex_symtab_index, dim, coords, buf, sz);
  else
...
cuda_api_read_local_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_local_memory (dev, sm, wp, ln, addr, buf, sz);
  else
                        ...
                        uint32_t regno, uint32_t *val)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_register (dev, sm, wp, ln, regno, val);
  else
...
cuda_api_read_pc (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t *pc)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_pc (dev, sm, wp, ln, pc);
  else
...
cuda_api_read_virtual_pc (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t *pc)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_virtual_pc (dev, sm, wp, ln, pc);
  else
...
cuda_api_read_lane_exception (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, CUDBGException_t *exception)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_lane_exception (dev, sm, wp, ln, exception);
  else
...
cuda_api_read_call_depth (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, int32_t *depth)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_call_depth (dev, sm, wp, ln, &api_call_depth); 
  else
...
cuda_api_read_syscall_call_depth (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, int32_t *depth)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_syscall_call_depth (dev, sm, wp, ln, &api_syscall_call_depth);
  else
                                      ...
                                      int32_t level, uint64_t *ra)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_read_virtual_return_address (dev, sm, wp, ln, api_call_level, ra); 
  else
...
cuda_api_write_global_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t addr, const void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_write_global_memory (dev, sm, wp, ln, addr, buf, sz);
  else
...
cuda_api_write_pinned_memory (uint64_t addr, const void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_write_pinned_memory (addr, buf, sz);
  else
...
cuda_api_write_param_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint64_t addr, const void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_write_param_memory (dev, sm, wp, addr, buf, sz);
  else
...
cuda_api_write_shared_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint64_t addr, const void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_write_shared_memory (dev, sm, wp, addr, buf, sz);
  else
...
cuda_api_write_local_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t addr, const void *buf, uint32_t sz)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_write_local_memory (dev, sm, wp, ln, addr, buf, sz);
  else
...
cuda_api_write_register (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint32_t regno, uint32_t val)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_write_register (dev, sm, wp, ln, regno, val);
  else
...
cuda_api_get_num_devices (uint32_t *numDev)
{
  ...
  *numDev = 0;
  ...
  if (cuda_remote)
    res = cuda_remote_api_get_num_devices (numDev);
  else
...
cuda_api_is_device_code_address (uint64_t addr, bool *is_device_address)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_is_device_code_address (addr, is_device_address);
  else
...
cuda_api_acknowledge_sync_events (void)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_acknowledge_sync_events ();
  else
...
cuda_api_disassemble (uint32_t dev, uint64_t addr, uint32_t *instSize, char *buf, uint32_t bufSize)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_disassemble (dev, addr, instSize, buf, bufSize);
  else
...
cuda_api_set_attach_state (cuda_attach_state_t state)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_clear_attach_state ();
  else
...
cuda_api_request_cleanup_on_detach (void)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_request_cleanup_on_detach();
  else
                                      ...
                                      uint64_t *address, ptxStorageKind *storage)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_memcheck_read_error_address (dev, sm, wp, ln, address, storage);
  else
...
cuda_api_get_grid_status (uint32_t dev, uint64_t grid_id, CUDBGGridStatus *status)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_get_grid_status (dev, grid_id, status);
  else
...
cuda_api_get_grid_info (uint32_t dev, uint64_t grid_id, CUDBGGridInfo *info)
{
  ...
  if (cuda_remote)
    res = cuda_remote_api_get_grid_info (dev, grid_id, info);
  else
