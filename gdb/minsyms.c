...
/* Add the minimal symbol SYM to an objfile's minsym lowercase hash table,
   TABLE.  */
static void
add_minsym_to_lowercase_hash_table (struct minimal_symbol *msym,
				   struct minimal_symbol **table)
{
  if (msym->lowercase_hash_next == NULL)
    {
      unsigned int hash
	= msymbol_lc_hash (MSYMBOL_SEARCH_NAME (msym)) % MINIMAL_SYMBOL_HASH_SIZE;
      msym->lowercase_hash_next = table[hash];
      table[hash] = msym;
    }
}
...
build_minimal_symbol_hash_tables (struct objfile *objfile)
{
  ...
  for ((i = objfile->per_bfd->minimal_symbol_count,
	...
	add_minsym_to_demangled_hash_table (msym,
      ...
      add_minsym_to_lowercase_hash_table (msym,
					  objfile->per_bfd->msymbol_lowercase_hash);
    }
}
